    ...
    if (token) {
      try {
        ...
        if (Array.isArray(usersResponse.data)) {
          ...
          setTotalPagesUsers(Math.ceil(usersResponse.data.length / ITEMS_PER_PAGE));
        } else {
...
const createUser = async () => {
  try {
    ...
    const confirmCreate = window.confirm('¿Seguro que quieres crear un nuevo usuario?');
...
const deleteUser = async (userId) => {
  try {
    ...
    const confirmDelete = window.confirm(`¿Seguro que quieres eliminar usuario con ID: ${userId}?`);
...
const viewUserByID = async (userId) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (action === 'create') {
        ...
        const response = await axios.put(`${API_BASE_URL}/users/${userData._id}`, userData, {
          headers: {
            'x-access-token': token
          }
        });
        ...
        const response = await axios.delete(`${API_BASE_URL}/users/${userData._id}`, {
          headers: {
            'x-access-token': token
          }
        });
  ...
  return (
    ...
    <div className="container mx-auto mt-8">
        ...
        <table className="w-full">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2" colSpan="3">Acciones CRUD</th>
            </tr>
          </thead>
          <tbody>
            {visibleUsers.map(user => (
              <tr key={user._id}>
                <td className="border px-4 py-2 max-w-[30px] overflow-x-auto">{user._id}</td>
                <td className="border px-4 py-2">{user.name}</td>
                <td className="border px-4 py-2">{user.lastname}</td>
                <td className="border px-4 py-2">{user.username}</td>
                <td className="border px-4 py-2 max-w-[30px] overflow-x-auto">{user.email}</td>
                <td className="border px-4 py-2">{user.mobile}</td>
                <td className="border px-4 py-2  max-w-[10px] overflow-x-auto">{user.password}</td>
                <td className="border px-4 py-2 max-w-[10px] overflow-x-auto">{user.roles}</td>
                <td className="border px-4 py-2">
                <button className="bg-blue-600 hover:bg-blue-900 text-white py-1 px-2 rounded ml-2" onClick={() => openModalUserView(user)}>Ver</button>

                </td>
                <td className="border px-4 py-2">
                <button className="bg-yellow-600 hover:bg-yellow-900 text-white py-1 px-2 rounded ml-2" onClick={() => editUser(user._id)}>Editar</button>
                </td>
               
                <td className="border px-4 py-2">
                <button className="bg-red-600 hover:bg-red-900 text-white py-1 px-2 rounded ml-2" onClick={() => deleteUser(user._id)}>Eliminar</button>
                </td>
      ...
      <button className="bg-green-600 hover:bg-green-900 text-white py-1 px-2 rounded ml-2" onClick={() => createUser()}>Crear Usuario</button>
    </div>
  ...
  <div className="fixed z-10 inset-0 overflow-y-auto">
    <div className="flex items-center justify-center min-h-screen">
      ...
      <div className="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all max-w-md w-full">
        {/* Cabecera del modal */}
        <div className="bg-gray-800 px-4 py-2">
          <h3 className="text-lg font-medium text-white">Vista Detallada de Usuario</h3>
        </div>
